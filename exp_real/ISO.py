...
async def main(args) -> None:
    ...
    id = args.robot
    hat_version = "v1"
    results_dir = f'./experiment_data/{id}'
    ...
    for pre_ind in range(run):
        c_trial = np.load(f'{results_dir}/{id}_{pre_ind}/state.npy', allow_pickle=True)
        f_trial = np.load(f'{results_dir}/{id}_{pre_ind}/fitnesses_trial.npy', allow_pickle=True)
        x_trial = np.load(f'{results_dir}/{id}_{pre_ind}/x_trial.npy', allow_pickle=True)
        c.append(c_trial)
    ...
    async with connect("10.15.3.100", "pi", "raspberry") as conn:
        ...
        while run < n_runs and not error:
            ...
            np.save(f'{results_dir}/{id}_{run}/state_con', log)
            np.save(f'{results_dir}/{id}_{run}/start_con', start_time.timestamp())
            ...
            np.save(f'{results_dir}/{id}_{run}/fitnesses_trial', f_trial)
            np.save(f'{results_dir}/{id}_{run}/x_trial', control_state[index])
            run += 1
