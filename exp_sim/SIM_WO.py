...
from sim_utils import set_controller, update_learner, save_states, update_robots
...
def simulate_robot(params, args):
    ...
    for skill in skills:
        ...
        for rep in range(n_reps):
            dir_results = f"./{results_dir}/{robot_name}/{skill}/{robot_name}{rep}"
            if not os.path.exists(dir_results):
                os.makedirs(dir_results)
            if os.path.isfile(f'{dir_results}/fitnesses.npy'):
            ...
            # %% Initialize learner
            learner = DifferentialEvolution(genomes, num_envs, params_learner['type'], (-1, 1), params_learner, output_dir=dir_results)
            ...
            np.save(f'{dir_results}/genomes.npy', genomes)
            np.save(f'{dir_results}/fitnesses.npy', np.array(fitnesses))
